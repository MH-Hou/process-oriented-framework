...
def main():
    ...
    experiment_data_dir = '/Users/ullrich/ullrich_ws/Projects/Greeting_Learning/experiment_data/human_feature_exp/random_collect/'
    ...
    with mp_pose.Pose(min_detection_confidence=0.5,
        ...
        for trial_id in range(total_trial_num):
            if (trial_id + 1) < error_trial_id:
                continue

            current_step = 0
            recorder.reset()

            agent.say("We are going to record a new way of greeting", role='performer')
            sleep(1.0)
            ...
            agent.say("Great, we finished the recording", role='performer')
            sleep(1.0)
            ...
            if trial_id < total_trial_num - 1:
                input("Press Enter to continue next section ...")

        agent.say("Excellent!",role='performer')
        sleep(0.5)
        agent.say("This part of experiment is finished!", role='performer')
        sleep(0.5)
        agent.say("Thanks for your patience and take a rest.", role='performer')
        agent.stop()
        cam.release()
        print("Random human-feature experiments finished")
